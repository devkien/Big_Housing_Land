...
{
    public static function getList(int $limit = 10, int $offset = 0, ?string $search = null)
    {
        $db = self::db();
        $params = [];

        $sql = "SELECT lr.*, c.ho_ten AS customer_name, c.so_dien_thoai AS customer_phone, u.ho_ten AS sender_name
    ...
    public static function countAll(?string $search = null)
    {
        $db = self::db();
        $params = [];
        $sql = "SELECT COUNT(*) FROM lead_reports lr LEFT JOIN customers c ON lr.customer_id = c.id WHERE 1=1";
        if ($search) {
            $sql .= " AND (c.ho_ten LIKE ? OR c.so_dien_thoai LIKE ? OR lr.note LIKE ? )";
            $like = '%' . $search . '%';
            $params[] = $like;
