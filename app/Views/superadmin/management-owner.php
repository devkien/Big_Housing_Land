    ...
    <div class="app-container" style="background: white;">
        ...
        <div class="pagination-container" style="text-align:center; margin-top:18px;">
            <?php if (isset($pages) && $pages > 1): ?>
                <?php if ($page > 1): ?>
                    <a class="page-link" href="<?= BASE_URL ?>/superadmin/management-owner?page=<?= $page - 1 ?><?= !empty($search) ? '&q=' . urlencode($search) : '' ?>">&lt;</a>
                <?php endif; ?>
                ...
                <?php for ($p = 1; $p <= $pages; $p++): ?>
                    ...
                    <?php else: ?>
                        <a class="page-link" href="<?= BASE_URL ?>/superadmin/management-owner?page=<?= $p ?><?= !empty($search) ? '&q=' . urlencode($search) : '' ?>"><?= $p ?></a>
                    <?php endif; ?>
                <?php endfor; ?>
