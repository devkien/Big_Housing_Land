...
<body>
    <div class="app-container" style="background: white;">
        ...
        <form action="<?= BASE_URL ?>/admin/management-resource-post" method="post" enctype="multipart/form-data">
            ...
            <div class="post-form-scroll" style="padding-bottom: 80px;">
                ...
                <div class="form-group" style="position: relative;">
                    <select id="select-district" name="quan_huyen" class="form-input" required>
                        <option value="" disabled selected>-- Chọn Quận / Huyện --</option>
                    </select>

                </div>
                <div class="form-group" style="position: relative;">
                    <select id="select-ward" name="xa_phuong" class="form-input" required>
    ...
    <script>
        (function() {
            ...
            const $dist = document.getElementById('select-district');
            ...
            function populateDistricts(data, provSlug) {
                clearSelect($dist, '-- Chọn Quận / Huyện --');
                clearSelect($ward, '-- Chọn Xã / Phường --');
                if (!provSlug || !data[provSlug]) return;
                const districts = data[provSlug].districts || {};
                Object.keys(districts).forEach(dslug => {
                    const opt = document.createElement('option');
                    opt.value = dslug;
                    opt.textContent = districts[dslug].name || dslug;
                    $dist.appendChild(opt);
                });
            }

            function populateWards(data, provSlug, distSlug) {
                clearSelect($ward, '-- Chọn Xã / Phường --');
                if (!provSlug || !distSlug || !data[provSlug]) return;
                const wards = (data[provSlug].districts[distSlug] || {}).wards || [];
                wards.forEach(w => {
                    const opt = document.createElement('option');
            ...
            $prov.addEventListener('change', function() {
                ...
                populateDistricts(window._locationsData || {}, prov);
            });

            $dist.addEventListener('change', function() {
                const dist = this.value;
                const prov = $prov.value;
                populateWards(window._locationsData || {}, prov, dist);
            });
