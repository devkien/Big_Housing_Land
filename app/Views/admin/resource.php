...
<body>
    ...
    <script>
        (function() {
                ...
                el.addEventListener('click', function(ev) {
                    ...
                    if (saveModal) {
                        ...
                        fetch(window.BASE_URL + '/admin/get-property-collections?id=' + window.currentPropertyId)
                            ...
                            .catch(function(e) { console.error('Lỗi tải dữ liệu:', e); });
                    }
                });
            });
            ...
            if (btnFilter) btnFilter.addEventListener('click', function() { filterModal.style.display = 'flex'; });

            var btnSearch = qs('#btn-search');
            if (btnSearch) btnSearch.addEventListener('click', function() { searchModal.style.display = 'flex'; });
                    ...
                    ?>
                        ...
                        ?>
                            <tr data-id="<?= $p['id'] ?>">
                                ...
                                <td style="cursor:pointer; color:#0b66ff;" onclick="window.location.href='<?= BASE_URL ?>/admin/detail?id=<?= htmlspecialchars($p['id']) ?>'"><?= $itemCode ?></td>
                                <td><?= $itemCreated ?></td>
        ...
        (function() {
            ...
            qsa('.icon-save').forEach(function(el) {
                el.addEventListener('click', function(ev) {
                    ...
                    if (saveModal) {
                        ...
                        // Gọi API để lấy danh sách BST đã lưu để tick sẵn (UX)
                        // Đường dẫn: /admin/get-property-collections
                        fetch(window.BASE_URL + '/admin/get-property-collections?id=' + currentPropertyId)
                            .then(function(r) { return r.json(); })
                            .then(function(data) {
                                if (data.success && data.collection_ids) {
                                    data.collection_ids.forEach(function(cid) {
                                        // Selector khớp với HTML name="collection[]"
                                        var cb = qs('#save-collection-modal input[name="collection[]"][value="' + cid + '"]');
                                        if (cb) cb.checked = true;
                                    });
                                }
            ...
            if (confirmSaveBtn) {
                confirmSaveBtn.addEventListener('click', function() {
                    ...
                    // Gọi API lưu
                    // Đảm bảo URL này khớp với Route bạn đã định nghĩa
                    fetch(window.BASE_URL + '/admin/save-to-collections', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                    ...
                    .then(function(json) {
                        if (json.ok || json.success) {
                            ...
                            // Cập nhật màu icon bookmark ngay lập tức
                            var tr = qs('tr[data-id="' + currentPropertyId + '"]');
                            if (tr) {
                                var icon = tr.querySelector('.icon-save');
                                if (icon) {
                                    if (selected.length > 0) {
