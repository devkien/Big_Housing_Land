    ...
    <div class="app-container" style="background: white;">
        ...
        <header class="resource-header">
            ...
            <div class="header-icon-btn">
                ...
                $collectionOwnerId = (int)($collection['user_id'] ?? 0);
                $canEdit = $isSuperAdmin || ($currentUserId > 0 && $currentUserId === $collectionOwnerId);
                ?>
                <?php if ($canEdit): ?>
                    <div class="btn-more-dots" data-id="<?= $collection['id'] ?>" data-name="<?= htmlspecialchars($collection['ten_bo_suu_tap']) ?>" style="cursor: pointer;"><i class="fa-solid fa-ellipsis"></i></div>
                <?php endif; ?>
            </div>
        </header>
        ...
        <div class="table-wrapper" style="margin-bottom: 0;">
            <table class="resource-table" style="min-width:1400px;">
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            <td colspan="18" style="text-align:center; padding:20px;">Không có tài nguyên nào trong bộ sưu tập này.</td>
                        </tr>
                        <?php else :
                        foreach ($items as $p) :
                        ...
                        ?>
                            ...
                            ?>
                            <tr data-id="<?= htmlspecialchars($p['id']) ?>" onclick="window.location.href='<?= BASE_URL ?>/admin/detail?id=<?= htmlspecialchars($p['id']) ?>'" style="cursor: pointer;">
                                <td style="padding-left:15px; cursor: default;" onclick="event.stopPropagation();">
                                    <?php if ($canEdit): ?>
                                    ...
                                    <?php else: ?>
                                        <i class="fa-solid fa-bookmark" style="color:#ffcc00; opacity: 0.7;" title="Đã lưu"></i>
                                    <?php endif; ?>
                                </td>
                                <td onclick="event.stopPropagation();"><i class="fa-regular fa-note-sticky icon-note"></i></td>
                                <td style="color:#0b66ff;"><?= $code ?></td>
                                <td><?= $created ?></td>
                                <td><?= $phong_ban ?></td>
                                <td><?= $tieu_de ?></td>
        ...
        <div id="filter-modal" class="modal">
            <div class="modal-content">
                ...
                <div class="filter-group">
                    ...
                    <input type="text" id="filter-address" class="filter-input" placeholder="Nhập mã tin, địa chỉ...">
                </div>
        ...
        <!-- Modal Tùy chọn Bộ sưu tập -->
        <div id="collection-options-modal" class="modal">
            <div class="modal-content">
                <h3 style="margin-bottom: 15px; font-size: 16px; text-align: center;">Tùy chọn</h3>
                <div style="display: flex; flex-direction: column; gap: 10px;">
                    <button id="edit-collection-btn" class="btn-save" style="background-color: #0033cc; width: 100%; margin: 0;">Sửa tên bộ sưu tập</button>
                    <button id="delete-collection-btn" class="btn-save" style="background-color: #ff3333; width: 100%; margin: 0;">Xóa bộ sưu tập</button>
                    <button id="cancel-options-btn" class="btn-cancel" style="width: 100%; text-align: center;">Hủy</button>
                </div>
            </div>
        </div>
        ...
        <div id="rename-collection-modal" class="modal">
            <div class="modal-content">
                ...
                <div class="filter-group">
                    <input type="text" id="rename-input" class="filter-input" placeholder="Nhập tên mới..." value="<?= htmlspecialchars($collection['ten_bo_suu_tap'] ?? '') ?>">
                </div>
                <div class="modal-actions">
                    <button id="cancel-rename-btn" class="btn-cancel">Hủy</button>
                    <button id="confirm-rename-btn" class="btn-apply">Lưu</button>
                </div>
            </div>
        </div>
        ...
        <script>
            (function() {
                // --- Logic xóa item khỏi bộ sưu tập ---
                ...
                // --- Logic Tùy chọn Bộ sưu tập (Sửa tên / Xóa) ---
                var optsModal = document.getElementById('collection-options-modal');
                var renModal = document.getElementById('rename-collection-modal');
                var dotsBtn = document.querySelector('.btn-more-dots');
                
                if (dotsBtn) {
                    dotsBtn.addEventListener('click', function(e) {
                        e.stopPropagation();
                        if (optsModal) optsModal.style.display = 'flex';
                    });
                }

                document.getElementById('cancel-options-btn')?.addEventListener('click', function() {
                    if (optsModal) optsModal.style.display = 'none';
                });

                document.getElementById('edit-collection-btn')?.addEventListener('click', function() {
                    if (optsModal) optsModal.style.display = 'none';
                    if (renModal) renModal.style.display = 'flex';
                    document.getElementById('rename-input')?.focus();
                });

                document.getElementById('cancel-rename-btn')?.addEventListener('click', function() {
                    if (renModal) renModal.style.display = 'none';
                });
                ...
                document.getElementById('confirm-rename-btn')?.addEventListener('click', function() {
                    ...
                    var fd = new FormData();
                    fd.append('id', <?= (int)($collection['id'] ?? 0) ?>);
                    fd.append('ten_bo_suu_tap', newName);
                    fetch('<?= BASE_URL ?>/admin/collection-rename', { method: 'POST', body: fd })
                ...
                document.getElementById('delete-collection-btn')?.addEventListener('click', function() {
                    ...
                    var fd = new FormData();
                    fd.append('id', <?= (int)($collection['id'] ?? 0) ?>);
                    fetch('<?= BASE_URL ?>/admin/collection-delete', { method: 'POST', body: fd })
                        .then(r => r.json()).then(d => {
                            if (d.ok) window.location.href = '<?= BASE_URL ?>/admin/collection';
                            else alert('Lỗi: ' + (d.error || 'Unknown'));
                        });
                });
            })();
